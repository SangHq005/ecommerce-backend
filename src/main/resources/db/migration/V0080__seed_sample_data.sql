...
SELECT 'admin@demo.local', 'Password123!', 'System Admin', 'ACTIVE', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM app_user WHERE email='admin@demo.local');
INSERT INTO app_user (email, password_hash, full_name, status, created_at, updated_at)
SELECT 'seller1@demo.local', 'Password123!', 'Seller One', 'ACTIVE', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM app_user WHERE email='seller1@demo.local');
INSERT INTO app_user (email, password_hash, full_name, status, created_at, updated_at)
SELECT 'seller2@demo.local', 'Password123!', 'Seller Two', 'ACTIVE', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM app_user WHERE email='seller2@demo.local');
INSERT INTO app_user (email, password_hash, full_name, status, created_at, updated_at)
SELECT 'client1@demo.local', 'Password123!', 'Client One', 'ACTIVE', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM app_user WHERE email='client1@demo.local');
INSERT INTO app_user (email, password_hash, full_name, status, created_at, updated_at)
SELECT 'client2@demo.local', 'Password123!', 'Client Two', 'ACTIVE', NOW(), NOW()
...
WHERE u.email IN ('seller1@demo.local', 'seller2@demo.local')
  AND NOT EXISTS (SELECT 1 FROM user_role ur WHERE ur.user_id = u.id AND ur.role_id = r.id);
INSERT INTO user_role (user_id, role_id)
SELECT u.id, r.id
FROM app_user u
JOIN role r ON r.code = 'CLIENT'
WHERE u.email IN ('client1@demo.local', 'client2@demo.local')
  AND NOT EXISTS (SELECT 1 FROM user_role ur WHERE ur.user_id = u.id AND ur.role_id = r.id);
...
FROM app_user u
WHERE u.email = 'client1@demo.local'
...
FROM app_user u
WHERE u.email = 'client2@demo.local'
  AND NOT EXISTS (SELECT 1 FROM user_profile p WHERE p.user_id = u.id);
...
WHERE s.sku_code = 'S24-128'
  AND u.email = 'seller2@demo.local'
...
WHERE p.name = 'iPhone 15 Pro'
  AND u.email = 'client1@demo.local'
