...
public class AuthService {
    ...
    public void changePassword(Long userId, String currentPassword, String newPassword) {
        ...
        // Revoke all refresh sessions except current one (force re-login on other devices)
        // Note: We don't revoke current session to avoid logging out the user
        // In production, you might want to revoke all sessions for security
        ...
        try {
            eventLogRepo.save(new EventLogDocument(
                    ...
                    Instant.now(),
                    null,
                    Map.of("email", user.getEmail(), "userId", userId)
            ));
        } catch (Exception e) {
            // Log warning but don't fail
        }
