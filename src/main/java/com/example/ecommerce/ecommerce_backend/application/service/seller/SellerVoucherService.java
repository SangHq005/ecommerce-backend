...
public class SellerVoucherService {
    ...
    @Transactional
    public SellerVoucherResponse duplicateVoucher(Long sellerId, Long voucherId, com.example.ecommerce.ecommerce_backend.api.dto.voucher.DuplicateVoucherRequest request) {
        SellerShopEntity shop = getShopBySeller(sellerId);
        SellerVoucherEntity original = getVoucherByIdAndShop(voucherId, shop.getId());
        
        if (voucherRepository.existsByShopIdAndCode(shop.getId(), request.code().toUpperCase())) {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR, "Voucher code already exists");
        }
        ...
        // Use PROTOTYPE pattern to clone the complex entity perfectly
        SellerVoucherEntity cloned = original.clone();
        
        // Override the basic fields for the new campaign
        cloned.setCode(request.code().toUpperCase());
        cloned.setName(request.name());
        cloned.setStartDate(request.startDate());
        cloned.setEndDate(request.endDate());
        
        cloned = voucherRepository.save(cloned);
        log.info("Duplicated voucher {} from original {}", cloned.getId(), original.getId());
        
        return SellerVoucherResponse.from(cloned);
    }
