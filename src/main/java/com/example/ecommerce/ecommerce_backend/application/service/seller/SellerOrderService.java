...
public class SellerOrderService {
    ...
    public void cancelOrder(Long shopId, Long sellerId, Long orderId, String reason) {
        ...
        eventPublisher.publishEvent(OrderStatusChangedEvent.of(
            orderEntity.getId(), orderEntity.getOrderCode(), orderEntity.getUserId(),
            OrderStatus.valueOf(previousStatus), OrderStatus.CANCELLED,
            sellerId, "SELLER"
        ));
    ...
    public OrderDetailResponse updateOrderStatus(Long shopId, Long sellerId, Long orderId, OrderStatus newStatus, String note, String trackingNumber) {
        ...
        eventPublisher.publishEvent(OrderStatusChangedEvent.of(
            orderEntity.getId(), orderEntity.getOrderCode(), orderEntity.getUserId(),
            OrderStatus.valueOf(previousStatus), newStatus,
            sellerId, "SELLER"
        ));
    ...
    public OrderDetailResponse markShipped(Long shopId, Long sellerId, Long orderId,
        ...
        eventPublisher.publishEvent(OrderStatusChangedEvent.of(
            orderEntity.getId(), orderEntity.getOrderCode(), orderEntity.getUserId(),
            OrderStatus.valueOf(previousStatus), OrderStatus.SHIPPED,
            sellerId, "SELLER"
        ));
    ...
    public OrderDetailResponse markDeliveryFailed(Long shopId, Long sellerId, Long orderId, String failedReason) {
        ...
        eventPublisher.publishEvent(OrderStatusChangedEvent.of(
            orderEntity.getId(), orderEntity.getOrderCode(), orderEntity.getUserId(),
            OrderStatus.valueOf(previousStatus), OrderStatus.DELIVERY_FAILED,
            sellerId, "SELLER"
        ));
    ...
    public OrderDetailResponse retryDelivery(Long shopId, Long sellerId, Long orderId) {
        ...
        eventPublisher.publishEvent(OrderStatusChangedEvent.of(
            orderEntity.getId(), orderEntity.getOrderCode(), orderEntity.getUserId(),
            OrderStatus.valueOf(previousStatus), OrderStatus.OUT_FOR_DELIVERY,
            sellerId, "SELLER"
        ));
    ...
    public List<OrderDetailResponse> batchShipOrders(Long shopId, Long sellerId, List<Long> orderIds) {
        ...
        for (OrderEntity order : orders) {
            try {
                ...
                eventPublisher.publishEvent(OrderStatusChangedEvent.of(
                    order.getId(), order.getOrderCode(), order.getUserId(),
                    OrderStatus.valueOf(currentStatus), OrderStatus.SHIPPED,
                    sellerId, "SELLER"
                ));
    ...
    public String generateBatchShippingLabels(Long shopId, Long sellerId, List<Long> orderIds) {
        ...
        for (OrderEntity order : orders) {
            ...
            // Fetch address - handle null addressId
            com.example.ecommerce.ecommerce_backend.infrastructure.persistence.mysql.entity.UserAddressEntity address;
            if (order.getAddressId() != null) {
                address = addressRepo.findById(order.getAddressId())
                    .orElse(new com.example.ecommerce.ecommerce_backend.infrastructure.persistence.mysql.entity.UserAddressEntity());
            } else {
                address = new com.example.ecommerce.ecommerce_backend.infrastructure.persistence.mysql.entity.UserAddressEntity();
            }
